        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h1 class="mb-4 text-2xl font-bold">{{ __('Create New Course') }}</h1>
                    ...
                    <form action="{{ route('courses.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-input id="courseName" type="text" name="courseName" class="block w-full mt-1" required />
                            <x-input-error for="courseName" class="mt-2" />
                        </div>

                        <!-- intro video Upload -->
                        <div class="mb-4">
                            <label for="intro_video" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Upload Video') }}</label>
                            <input type="file" name="intro_video" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" accept="video/*" required>
                        </div>

                        <!-- Course Description -->
                        <div>
                            <x-label for="courseDescription" value="{{ __('Course Description') }}" />
                            <textarea name="courseDescription" id="courseDescription" class="block w-full mt-1 bg-white rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-900 dark:text-white" rows="4" required></textarea>
                        ...
                        <div>
                            ...
                            <select name="track_id" id="track_id" class="block w-full mt-1 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 dark:bg-gray-900 dark:text-white" required>
                                <option value="" disabled selected>{{ __('Select a Track') }}</option>
                                @foreach ($traks as $trak)
                                    <option value="{{ $trak->id }}">{{ $trak->trackName }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-input id="coursePhoto" type="file" name="coursePhoto" class="block w-full mt-1" accept="image/*" required />
                        ...
                        <div>
                            ...
                            <x-input id="Price" type="number" name="Price" min="0" class="block w-full mt-1" required />
